...
public class ClientForgotPassword extends JFrame {
	...
	public ClientForgotPassword() {
		...
		btnNewButton.setBounds(413, 430, 148, 21);
				...
				try {
					if(email.isEmpty() || newPassword.isEmpty() || confirmPassword.isEmpty()) {
						...
						if(newPassword.equals(confirmPassword)) {
							ResultSet rs= Select.getData("select * from clientCredentials where email='"+email+"'");//because email is unique for each receptionist
							try {
								if(rs.next()) {
		...
		lblNewLabel_9.setBounds(278, 109, 100, 100);
		contentPane.add(lblNewLabel_9);
		
		passwordField = new JPasswordField();
		passwordField.setBounds(415, 302, 300, 19);
		contentPane.add(passwordField);
		
		passwordField_1 = new JPasswordField();
		passwordField_1.setBounds(415, 363, 300, 19);
		...
		btnCancel.setBounds(577, 430, 148, 21);
